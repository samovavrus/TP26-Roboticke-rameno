...
Očakávané správanie systému z používateľského hľadiska je také, že rameno po spustení funguje ako samostatný vstavaný systém (embedded device) bez nutnosti stáleho pripojenia k počítaču. Používateľ prostredníctvom ovládacieho panelu volí režim riadenia a plynule zadáva cieľovú pozíciu koncového efektora (X, Y, Z súradnice), jeho orientáciu (Roll, Pitch, Yaw), alebo natáča jednotlivé kĺby priamo. Systém následne automaticky naplánuje pohyb a manipulátor sa plynule, bezpečne a bez kolízií presunie do požadovaného stavu.
